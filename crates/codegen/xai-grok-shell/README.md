...
|---|---|---|
| **What it is** | A headless refresh over a credential Grok already holds — near-expiry rotation, or a token the server rejected | A sign-in: `open-grok login`, the sign-in screen, or the escalation after a headless run couldn't mint |
...
> **Note:** Only `[mcp_servers]` is supported in project-scoped `.opengrok/config.toml`. Other config sections (models, etc.) are only read from `~/.opengrok/config.toml`.
