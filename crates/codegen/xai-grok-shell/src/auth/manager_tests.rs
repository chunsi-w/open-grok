...
#[test]
#[serial_test::serial(force_dark_wake_env)] // reads OPENGROK_AUTH_FORCE_DARK_WAKE
fn is_dark_wake_false_when_power_listener_not_started() {
    let _unset = xai_grok_test_support::EnvGuard::unset("OPENGROK_AUTH_FORCE_DARK_WAKE");
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), GrokComConfig::default());
    assert!(
        !mgr.is_dark_wake(),
...
/// `OPENGROK_AUTH_FORCE_DARK_WAKE` forces the dark-wake answer for manual and
...
fn is_dark_wake_env_override_forces_both_states() {
    ...
    {
        let _g = EnvGuard::set("OPENGROK_AUTH_FORCE_DARK_WAKE", "1");
        assert!(
            mgr.is_dark_wake(),
    ...
    {
        let _g = EnvGuard::set("OPENGROK_AUTH_FORCE_DARK_WAKE", "0");
    ...
    }
    {
        ...
        let _g = EnvGuard::set("OPENGROK_AUTH_FORCE_DARK_WAKE", "yes");
