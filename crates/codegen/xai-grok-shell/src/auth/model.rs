...
/// auth.json scope key for plain API key auth (desktop login, `open-grok login --api-key`).
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from grok-desktop login or `open-grok login --api-key`)
    ApiKey,
}
...
impl GrokAuth {
    ...
    /// `true` when the token comes from a first-party xAI account -- either an
    /// OIDC login against https://auth.x.ai (or the local-dev equivalent), or
    /// an external auth provider that declared an xAI issuer for its token.
    ///
    /// The issuer is a client-side hint, not a trust assertion: everything it
    /// unlocks still authenticates the actual token server-side, and it never
    /// influences endpoints.
    ...
    /// (`is_xai_auth`), matching the built-in devbox login they replace. Plain
    /// API keys never do.
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `open-grok login --legacy`
