...
/// Machine-readable channel name derived from the release pointer cache.
///
/// Reads `stable_version` from `~/.opengrok/version.json` (written by the
...
/// Open Grok releases encode the fork revision in SemVer's prerelease
/// component (`0.1.220-open-grok.29`), which plain SemVer orders BELOW the
/// bare upstream core `0.1.220`.
pub fn is_open_grok_version(version: &Version) -> bool {
    version
        .pre
        .as_str()
        .split('.')
...
/// Whether `current` satisfies `floor` under Open Grok floor semantics: a bare
/// floor is compared against the core tuple for fork builds, so a fork release
/// from that core satisfies it. Explicit prerelease floors — including
/// `open-grok.N` floors — retain full SemVer ordering.
pub fn version_satisfies_floor(current: &Version, floor: &Version) -> bool {
    if is_open_grok_version(current) && floor.pre.is_empty() {
        (current.major, current.minor, current.patch) >= (floor.major, floor.minor, floor.patch)
    } else {
        current >= floor
    }
}
...
impl VersionPolicy {
    ...
    /// Open Grok fork targets satisfy a bare floor by core version.
    fn skips_update_target(&self, target: &str) -> bool {
        matches!(
            (&self.minimum, Version::parse(target)),
            (Some(min), Ok(t)) if !version_satisfies_floor(&t, min)
        )
    }
...
mod tests {
    ...
    #[test]
    fn open_grok_fork_releases_satisfy_bare_floors_by_core_version() {
        assert!(version_satisfies_floor(
            &v("0.1.220-open-grok.4"),
            &v("0.1.220")
        ));
        assert!(version_satisfies_floor(
            &v("0.1.220-open-grok.4"),
        ...
        // Explicit prerelease floors keep full SemVer ordering.
        assert!(version_satisfies_floor(
            &v("0.1.220-open-grok.4"),
            &v("0.1.220-open-grok.3")
        ));
        assert!(!version_satisfies_floor(
            &v("0.1.220-open-grok.3"),
            &v("0.1.220-open-grok.4")
        ));
        // Non-fork prereleases keep plain SemVer.
        assert!(!version_satisfies_floor(
            &v("0.1.220-alpha.4"),
            &v("0.1.220")
        ));

        // The updater must not skip a same-core fork target on a bare soft
        // floor, and a bare hard floor must not rewrite a satisfying fork
        // target to a nonexistent bare release tag.
        let p = pol(Some("0.1.220"), None, None, None);
        assert_eq!(
            p.resolve_target("0.1.220-open-grok.30"),
            Some("0.1.220-open-grok.30".into())
        );
        let hard = pol(None, None, Some("0.1.220"), None);
        assert_eq!(hard.clamp("0.1.220-open-grok.30"), "0.1.220-open-grok.30");
    }
