...
pub(crate) async fn submit_feedback_workflow(
    ...
    // Open Grok privacy: strip session context/metadata and the raw feedback
    // text before sending to the backend. The full submission is already
    // persisted locally above; only the outbound copy is redacted.
    ...
    pub(crate) async fn submit_text_feedback(
        ...
        submit_feedback_workflow(
            ...
            SubmitFeedbackOptions {
                // solicited: slash command isn't responding to a request
                solicited: false,
                telemetry_enabled: telemetry_enabled && self.allows_xai_export(),
                author_identity,
            },
        )
        .await
...
mod author_identity_tests {
    ...
    /// resolved, and carried on the feedback POST. Author identity survives the
    /// Open Grok provider-boundary strip (the opt-in deployment stays reachable),
    /// while the model metadata and raw feedback text are stripped before the
    /// wire and retained only on the local entry.
    #[tokio::test]
    #[serial_test::serial]
...
"#,
        ...
        // Author identity is preserved across the provider boundary so the
        // opted-in deployment stays reachable, but Open Grok's privacy strip
        // removes the model metadata and the raw feedback text before the POST.
        ...
        assert!(
            body["modelId"].is_null(),
            "model id must be stripped before the wire POST"
        );
        assert!(
            body["feedbackText"].is_null(),
            "raw feedback text must be stripped before the wire POST"
        );

        // The local entry keeps the author fields and the full pre-strip
        // context (model id and raw feedback text included).
    ...
    /// `GROK_USER_METADATA` is merged into the submission and retained on the
    /// local feedback.jsonl entry, but Open Grok's provider-boundary privacy
    /// strip removes it from the outbound wire body.
    #[tokio::test]
    #[serial_test::serial]
    async fn workflow_merges_user_metadata_into_submission() {
        ...
        // Merged metadata is persisted locally but stripped from the wire body
        // by the provider-boundary privacy strip.
        let body = captured.lock().clone().expect("server saw the POST");
        assert!(
            body["metadata"].is_null(),
            "user metadata must be stripped before the wire POST"
        );
