...
/// `cached_token` method, `NotByok` model facts, and the supplied auth manager.
///
/// Open Grok's session-token gate requires a first-party host even for
/// `NotByok` (fork hardening vs upstream). These budget tests opt into trusting
/// the loopback mock so recovery/resubmit can be exercised end-to-end.
async fn session_token_actor(
    ...
    // Definite NotByok plus the test-only loopback trust hook (see
    // `TrustLoopbackSessionAuth`): Open Grok still requires first-party for
    // session-token refresh in production, including `NotByok`.
    actor
        .model_auth_memo
...
/// RAII: Open Grok withholds session-token refresh from loopback by default;
/// these end-to-end budget tests temporarily trust the mock host.
struct TrustLoopbackSessionAuth;
impl TrustLoopbackSessionAuth {
    fn enable() -> Self {
        crate::session::acp_session::trust_loopback_session_auth_for_tests(true);
        Self
    }
}
impl Drop for TrustLoopbackSessionAuth {
    fn drop(&mut self) {
        crate::session::acp_session::trust_loopback_session_auth_for_tests(false);
    }
}
...
fn fail_closed_401_is_uncharged_and_turn_survives() {
    // Full `handle_prompt` futures overflow libtest's default stack in debug.
