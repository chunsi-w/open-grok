...
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(
        ...
        // Strip reasoning/thinking blocks only where the backend rejects them
        // (Anthropic Messages requires a `thinking` config with thinking blocks).
    ...
    async fn side_question_prompt_and_tools(
        ...
        let provider = self
            ...
            .unwrap_or_default();
        let tool_specs =
            self.turn_base_tool_specs(&self.prepare_tool_definitions().await, provider);
...
#[cfg(test)]
mod side_question_retry_tests {
    use super::should_retry_side_question;
