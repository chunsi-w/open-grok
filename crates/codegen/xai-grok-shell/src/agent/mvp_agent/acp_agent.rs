...
pub(super) fn wafer_models_apply_payload(
    ...
    match refreshed {
        Ok(refreshed) => serde_json::json!({ "refreshed": refreshed, "models": models }),
        Err(warning) => {
            tracing::warn!(%warning, "Wafer model query failed; returning current models");
            serde_json::json!({ "refreshed": false, "warning": warning, "models": models })
        }
    }
}
        ...
        let result = match method.as_ref() {
            ...
            "open-grok/wafer/models/apply" => {
                ...
                if cancelled_subagents > 0 {
                    tracing::warn!(
                        cancelled_subagents,
                        "cancelled subagents before Wafer runtime credential change"
                    );
                }
                let refreshed = self
                    .models_manager
                    .apply_wafer_credential_change()
                    .await
                    .map_err(|error| error.to_string());
                let available = self.models_manager.available();
                let models = acp::SessionModelState::new(
