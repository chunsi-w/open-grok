...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    // Project `.opengrok/lsp.json`.
    if cwd.join(".opengrok").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    if path_present_or_uncertain(&hook_root.join(".opengrok").join("hooks"))
    ...
    // that must be gated — else a plugin clone (e.g. `.opengrok/plugins/evil/`, even
    ...
    // Project AGENT dirs (`.opengrok/agents` / `.claude/agents`): a project agent
    ...
    if directory_present_or_uncertain(&hook_root.join(".opengrok").join("workflows")) {
        hit!("workflows");
    }
...
mod tests {
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.opengrok/agents`-only clone must be gated: a project agent definition
        // can carry an inline `hooks:` block (code-exec) and can shadow a built-in
        // subagent by name.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".opengrok").join("agents")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_roles() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".opengrok").join("roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_personas() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".opengrok").join("personas")).unwrap();
    ...
    fn project_subagent_marker_at_repo_root_is_absent_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".opengrok/roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_workflows_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".opengrok").join("workflows")).unwrap();
        let subdir = tmp.path().join("crates").join("inner");
        std::fs::create_dir_all(&subdir).unwrap();
        assert!(repo_configs_present(&subdir));
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".opengrok").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".opengrok").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.opengrok/agents` — even when launched from a SUBDIR (the cwd→git-root walk
