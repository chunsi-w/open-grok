...
pub const WORKSPACE_MODE_LOG: &str = "opengrok.pager.workspace_mode";
...
mod apply_tests {
    use super::*;
    use crate::app::session_startup::{
        LocalWorkspaceMode, OPENGROK_CHAT_LOCAL_WORKSPACE_ACK_ENV, set_active_local_workspace,
    };
    ...
    fn welcome_local_one_shot_only_when_agents_alive() {
        let _ack = xai_grok_test_support::EnvGuard::set(OPENGROK_CHAT_LOCAL_WORKSPACE_ACK_ENV, "1");
        set_active_local_workspace(None).unwrap();
        let tmp = tempfile::tempdir().unwrap();
        let out = prepare_welcome_workspace_for_new_session(
            WelcomeWorkspaceMode::LocalWorkspace,
    ...
    fn local_without_ack_awaits_confirm() {
        ...
        // `grok_home()` caches OPENGROK_HOME in a OnceLock, so a temp-home
        // EnvGuard cannot redirect the ack path after the first resolve.
        // Clear the ack file under the process-effective home instead.
        let ack = xai_grok_shell::util::grok_home::grok_home().join("local_workspace_ack");
