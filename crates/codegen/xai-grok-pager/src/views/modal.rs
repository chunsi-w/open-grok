...
///
/// `sharing_enabled` controls whether `/share` is included. `slash` exposes
/// the render mode (draft-preserving external-editor row only in minimal
/// mode) and the registry (`mode_support`) so rows the current mode can't
/// run are dropped.
