...
/// unless `OPENGROK_CHAT_LOCAL_WORKSPACE_MODE` / attach server id is set.
#[cfg(feature = "local-workspace")]
pub const OPENGROK_CHAT_LOCAL_WORKSPACE_ENV: &str = "OPENGROK_CHAT_LOCAL_WORKSPACE";
#[cfg(feature = "local-workspace")]
pub const OPENGROK_CHAT_LOCAL_WORKSPACE_CWD_ENV: &str = "OPENGROK_CHAT_LOCAL_WORKSPACE_CWD";
#[cfg(feature = "local-workspace")]
pub const OPENGROK_CHAT_LOCAL_WORKSPACE_MODE_ENV: &str = "OPENGROK_CHAT_LOCAL_WORKSPACE_MODE";
#[cfg(feature = "local-workspace")]
pub const OPENGROK_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV: &str =
    "OPENGROK_CHAT_LOCAL_WORKSPACE_SERVER_ID";
#[cfg(feature = "local-workspace")]
pub const OPENGROK_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV: &str =
    "OPENGROK_CHAT_LOCAL_WORKSPACE_ALLOW_HOME";
/// Skip interactive first-run confirm (still prints the banner).
#[cfg(feature = "local-workspace")]
pub const OPENGROK_CHAT_LOCAL_WORKSPACE_ACK_ENV: &str = "OPENGROK_CHAT_LOCAL_WORKSPACE_ACK";
...
pub const LOCAL_WORKSPACE_ATTACH_NEEDS_SERVER_ID: &str = "local-workspace attach requires --local-workspace-attach=<server_id> \
     (or OPENGROK_CHAT_LOCAL_WORKSPACE_SERVER_ID)";
#[cfg(feature = "local-workspace")]
...
pub const LOCAL_WORKSPACE_HOME_DENIED: &str =
    "local-workspace cwd may not be / or $HOME unless OPENGROK_CHAT_LOCAL_WORKSPACE_ALLOW_HOME=1";
#[cfg(feature = "local-workspace")]
...
#[cfg(feature = "local-workspace")]
pub const LOCAL_WORKSPACE_ACK_REQUIRED: &str = "local-workspace requires interactive confirm, OPENGROK_CHAT_LOCAL_WORKSPACE_ACK=1, or an ack file";
/// Declared advertised tool ids for attach FS-only check (comma-separated).
#[cfg(feature = "local-workspace")]
pub const OPENGROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV: &str =
    "OPENGROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS";
#[cfg(feature = "local-workspace")]
...
pub fn resolve_local_workspace_config(
    ...
    let env_enable = env_truthy(OPENGROK_CHAT_LOCAL_WORKSPACE_ENV);
    let env_mode = env_nonempty(OPENGROK_CHAT_LOCAL_WORKSPACE_MODE_ENV);
    let env_server_id = env_nonempty(OPENGROK_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    let env_cwd = env_nonempty(OPENGROK_CHAT_LOCAL_WORKSPACE_CWD_ENV).map(std::path::PathBuf::from);
    ...
    let mode = if cli_attach.is_some() {
        ...
        match m.as_str() {
            ...
            other => {
                anyhow::bail!(
                    "invalid {OPENGROK_CHAT_LOCAL_WORKSPACE_MODE_ENV}={other:?}; expected own|attach"
                )
            }
        }
...
pub fn validate_local_workspace_cwd(path: &std::path::Path) -> anyhow::Result<std::path::PathBuf> {
    ...
    if env_truthy(OPENGROK_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV) {
        return Ok(canon);
    }
...
///
/// Skip confirm only with `OPENGROK_CHAT_LOCAL_WORKSPACE_ACK=1` or a prior ack file.
...
pub fn local_workspace_ack_satisfied() -> bool {
    if env_truthy(OPENGROK_CHAT_LOCAL_WORKSPACE_ACK_ENV) {
        return true;
    }
...
/// via `OPENGROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS` (comma-separated ids).
/// Unset / empty → refuse.
#[cfg(feature = "local-workspace")]
...
pub fn probe_advertised_tool_ids() -> Option<Vec<String>> {
    let raw = env_nonempty(OPENGROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV)?;
...
fn local_workspace_ack_path() -> Option<std::path::PathBuf> {
    Some(xai_grok_shell::util::grok_home::grok_home().join("local_workspace_ack"))
}
    ...
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'open-grok' to start a new session."
        )
    })?;
...
mod tests {
    ...
    fn advertised_tools_env() -> xai_grok_test_support::EnvGuard {
        xai_grok_test_support::EnvGuard::set(
            OPENGROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV,
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(OPENGROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[test]
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(OPENGROK_CHAT_LOCAL_WORKSPACE_CWD)]
    #[serial_test::serial(OPENGROK_CHAT_LOCAL_WORKSPACE)]
    #[serial_test::serial(OPENGROK_CHAT_LOCAL_WORKSPACE_MODE)]
    #[serial_test::serial(OPENGROK_CHAT_LOCAL_WORKSPACE_SERVER_ID)]
    #[test]
    fn resolve_local_workspace_cwd_only_is_not_a_request() {
        let tmp = tempfile::tempdir().unwrap();
        let _cwd = xai_grok_test_support::EnvGuard::set(
            OPENGROK_CHAT_LOCAL_WORKSPACE_CWD_ENV,
            tmp.path().to_str().unwrap(),
        );
        let _enable = xai_grok_test_support::EnvGuard::unset(OPENGROK_CHAT_LOCAL_WORKSPACE_ENV);
        let _mode = xai_grok_test_support::EnvGuard::unset(OPENGROK_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid =
            xai_grok_test_support::EnvGuard::unset(OPENGROK_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_own_env_defaults() {
        let _env = advertised_tools_env();
        let _enable = xai_grok_test_support::EnvGuard::set(OPENGROK_CHAT_LOCAL_WORKSPACE_ENV, "1");
        let _mode = xai_grok_test_support::EnvGuard::unset(OPENGROK_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid =
            xai_grok_test_support::EnvGuard::unset(OPENGROK_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(OPENGROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[serial_test::serial(OPENGROK_CHAT_LOCAL_WORKSPACE_ALLOW_HOME)]
    ...
    fn resolve_local_workspace_defaults_cwd_and_denies_home() {
        ...
        let _allow =
            xai_grok_test_support::EnvGuard::unset(OPENGROK_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV);
    ...
    fn resolve_local_workspace_refuses_uncheckable_toolset() {
        let _tools = xai_grok_test_support::EnvGuard::unset(
            OPENGROK_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV,
        );
        let tmp = tempfile::tempdir().unwrap();
        let err =
    ...
    fn local_workspace_non_tty_requires_ack() {
        let _ack = xai_grok_test_support::EnvGuard::unset(OPENGROK_CHAT_LOCAL_WORKSPACE_ACK_ENV);
        let home = tempfile::tempdir().unwrap();
        let _home =
