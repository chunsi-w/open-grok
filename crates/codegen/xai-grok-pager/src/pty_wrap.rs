//! Local PTY wrapper: the engine behind `open-grok wrap` (see [`crate::wrap_cmd`]).
//!
...
///
/// This is the engine behind `open-grok wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // `open-grok` reached over SSH — that its OSC 52 clipboard writes are being
    // intercepted here and copied to the real local clipboard. The inner Open Grok
    ...
    // which is the usual SSH case (only `TERM` propagates, so the inner Open Grok
    ...
    // detached — `open-grok wrap` is short-lived and exits with the child.
