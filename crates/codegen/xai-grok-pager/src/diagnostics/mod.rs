    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "Grok also saves each copy to the backup file shown in the copy message. To copy \
             directly, run `open-grok wrap ssh <host>` on your local computer or use a terminal that \
...
/// through `open-grok wrap ssh <host>` on the user's local machine, which gives a
...
mod tests {
    ...
    // -- ssh_wrap_hint: `open-grok wrap ssh` recommendation --------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("open-grok wrap ssh <host>"));
        assert!(
