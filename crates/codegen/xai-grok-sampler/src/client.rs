...
impl SamplingClient {
            ...
            Some(resolver) => resolver
                ...
                .or_else(|| {
                    (!resolver.fail_closed_on_missing())
                        .then(|| {
                            Self::sent_fragment_from_headers(
    ...
    /// 401 response. Each UNAUTHORIZED arm in this file calls this helper
    /// immediately before returning `SamplingError::Auth(...)`. Emit happens
...
mod tests {
    ...
    fn post_capture_is_immune_to_resolver_rotation_after_build() {
        ...
        let auth = request
            ...
            .get(AUTHORIZATION)
            .and_then(|v| v.to_str().ok());
        assert_eq!(auth, Some("Bearer rejected-token-oldtail1"));
    ...
    /// expected `consumer` and the `post()`-captured fragment. The key
    /// assertion is that the callback receives the fragment only -- the
